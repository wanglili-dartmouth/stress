...
class SM2Vec():
    def __init__(self, graph, walk_length=10, num_walks=100, workers=1, verbose=0, stay_prob=0.3, opt1_reduce_len=True, opt2_reduce_sim_calc=False, opt3_num_layers=None, temp_path='./temp/', reuse=False):
        self.graph = graph
        ...
        self.pair_distances=self.create_context_graph(self.opt3_num_layers, workers, verbose)
    ...
    def get_embeddings(self,alpha,max_layer,dim=2):
        dis=np.zeros((len(list(self.graph.nodes)), len(list(self.graph.nodes))))
        layers_distances=self.pair_distances
        for d1 in self.idx:
            for d2 in self.idx:
                if(d1==d2):
                    dis[int(d1)][int(d2)]=0
                else:
                    if((int(d1),int(d2))in layers_distances):
                        dis[int(d1)][int(d2)]=layers_distances[(int(d1),int(d2))][max_layer]**alpha
                    else:
                        dis[int(d1)][int(d2)]=layers_distances[(int(d2),int(d1))][max_layer]**alpha

        tmp=MDS(n_components=dim,dissimilarity='precomputed',random_state=0).fit_transform(dis)
        embeddings={}
        for i in range(len(tmp)):
            embeddings[self.idx2node[i]]=tmp[i]
        return embeddings

...
def test_grid(labels,embedding_dict):
    ...
    clf = svm.SVC( C=10,gamma=1, random_state=0)
...
if __name__ == "__main__":
    random.seed(0)
    np.random.seed(0)
    graph = nx.read_weighted_edgelist("data/usa-airports.edgelist", delimiter=" ", nodetype=None,create_using=nx.Graph())
    graph_int = nx.read_weighted_edgelist("data/usa-airports.edgelist", delimiter=" ", nodetype=int,create_using=nx.Graph())
    labels = pd.read_csv('data/labels-usa-airports.txt', index_col=0, sep=" ")
